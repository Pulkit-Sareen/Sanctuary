...
/* Torch Reveal — from Motiq (https://motiq.dev/components/torch-reveal).
            ...
            split
              ? // Static split: a hard 55/45 comparison with a dashed divider —
                // the "two layers" story survives with no pointer tracking at all.
                {
                  ...
                  borderLeft: "1.5px dashed var(--motiq-secondary-accent, #22c7d9)",
                  background: revealClassName ? undefined : "var(--motiq-bg, #080c14)",
                }
              : {
                  maskImage,
      ...
      {staticMode ? null : (
        <div
          ...
          style={{
            ...
            background:
              "radial-gradient(circle, color-mix(in oklab, var(--motiq-secondary-accent, #22c7d9) 16%, transparent), transparent 62%)",
          }}
        />
      )}
    </div>
...
export function TorchReveal(props: TorchRevealProps) {
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: MOTIQ_TOKENS }} />
      <TorchRevealBase {...props} />
    </>
  );
}
